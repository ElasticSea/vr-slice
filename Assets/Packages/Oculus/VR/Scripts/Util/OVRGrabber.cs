...
public class OVRGrabber : MonoBehaviour
{
    ...
    {
        GetComponent<Rigidbody>().MovePosition(m_parentTransform.position);
        GetComponent<Rigidbody>().MoveRotation(m_parentTransform.rotation);

        if (!m_parentHeldObject)
        {
            MoveGrabbedObject(m_parentTransform.position, m_parentTransform.rotation);
        }
        m_lastPos = transform.position;
        m_lastRot = transform.rotation;
    ...
    protected virtual void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
    {
        ...
        if (forceTeleport)
        {
            grabbedRigidbody.transform.position = pos;
            grabbedRigidbody.transform.rotation = rot;
        }
        else
        {
            grabbedRigidbody.MovePosition(pos);
